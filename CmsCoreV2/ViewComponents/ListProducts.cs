...
namespace CmsCoreV2.ViewComponents
{
    public class ListProducts : ViewComponent
    {
        ...
        public List<Product> GetProducts(string[] categories, int count)
        {
            ...
            {
                return (from pc in _context.ProductProductCategories join p in _context.Products.Include(i=>i.ProductMedias)  on pc.ProductId equals p.Id join c in _context.ProductCategories on pc.ProductCategoryId equals c.Id where (categories.Length > 0 ? categories.Contains(c.Name.ToLower()) : true) && p.IsApproved == true  && (p.CatalogVisibility == CatalogVisibility.VisibilityBoth || p.CatalogVisibility == CatalogVisibility.VisibilityCatalog) orderby p.CreateDate descending select p).Take(count).ToList();
            }
            else
            {
                return (from p in _context.Products.Include(i=>i.ProductMedias)  where p.IsApproved == true  && (p.CatalogVisibility == CatalogVisibility.VisibilityBoth || p.CatalogVisibility == CatalogVisibility.VisibilityCatalog) orderby p.CreateDate descending select p).Take(count).ToList();
            }
        }
