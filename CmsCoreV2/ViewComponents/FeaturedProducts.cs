...
namespace CmsCoreV2.ViewComponents
{
    public class FeaturedProducts : ViewComponent
    {
        ...
        public List<Product> GetFeaturedProducts(string[] categories, int count)
        {
            ...
            {
                return (from pc in _context.ProductProductCategories join p in _context.Products on pc.ProductId equals p.Id join c in _context.ProductCategories on pc.ProductCategoryId equals c.Id where (categories.Length > 0 ? categories.Contains(c.Name.ToLower()) : true) && p.IsFeatured == true && p.IsApproved == true  && (p.CatalogVisibility == CatalogVisibility.VisibilityBoth || p.CatalogVisibility == CatalogVisibility.VisibilityCatalog) orderby p.CreateDate descending select p).Take(count).ToList();
            }
            else
            {
                return (from p in _context.Products where p.IsFeatured==true && p.IsApproved == true  && (p.CatalogVisibility == CatalogVisibility.VisibilityBoth || p.CatalogVisibility == CatalogVisibility.VisibilityCatalog) orderby p.CreateDate descending select p).Take(count).ToList();
            }
        }
